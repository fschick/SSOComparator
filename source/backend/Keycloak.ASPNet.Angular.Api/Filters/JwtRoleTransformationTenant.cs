...
public class JwtRoleTransformationTenant : IClaimsTransformation
{
    ...
    public Task<ClaimsPrincipal> TransformAsync(ClaimsPrincipal principal)
    {
        ...
        var realmAccess = principal.FindFirst("urn:zitadel:iam:org:project:roles")?.Value;
        if (string.IsNullOrWhiteSpace(realmAccess))
            return Task.FromResult(result);

        var roleContainer = JsonConvert.DeserializeObject<ZitadelJwtGlobalRoles>(realmAccess);
        if (roleContainer == null)
            return Task.FromResult(result);

        var realmRoles = roleContainer.Keys
